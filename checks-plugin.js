...
const CONFIG = Object.freeze({
  ...
  API_VERSION: "3.12.0",
  FIRMWARE_CI_API_URL: "https://api.firmwareci.9esec.dev:8444/v0",
...
class FirmwareChecks {
  ...
  createJobRequestRun(jobRequest, changeData) {
    ...
    const run = {
      ...
      actions: [this.createRerunAction(requestId, "job-request")],
      results: [],
    };
  ...
  /**
   * Creates a rerun action for a job request or job
   * @param {string} id - ID of the job request or job
   * @param {string} type - Type of the entity ('job-request' or 'job')
   * @returns {Object} - Action object for Gerrit
   */
  createRerunAction(id, type) {
    const endpoint = type === "job" ? "job" : "job-request";
    const entityId = id;

    return {
      name: "RERUN",
      tooltip: `Rerun this ${type}`,
      primary: true,
      summary: type === "job-request",
      callback: async (change, patchset, attempt, externalId, checkName) => {
        try {
          const response = await fetch(
            `${CONFIG.FIRMWARE_CI_API_URL}/${endpoint}/rerun/${entityId}`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
          ...
          if (!response.ok) {
            return this.handleRerunError(response);
          }

          // Try to parse the response JSON for more details
          let responseData = null;
          try {
            responseData = await response.json();
          } catch (e) {
            // Response wasn't JSON, continue with default message
          }

          return {
            message:
              responseData?.message ||
              `Successfully triggered rerun of "${checkName}"`,
            shouldReload: true,
          };
        } catch (error) {
          console.error(`Error rerunning ${type}:`, error);
          return {
            message: `Error rerunning ${type}: ${error.message}`,
            shouldReload: false, // Don't reload on errors
          };
        }
      },
    };
  ...
  createJobResult(job, attempt) {
    ...
    return {
      ...
      actions: [this.createRerunAction(job.id, "job")],
    };
  }
